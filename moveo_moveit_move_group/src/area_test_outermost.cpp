...
void motor_setep_convert(Eigen::VectorXd &data){
  static const double joint_division[5] = {200*32*10.533*0.95, 200*16*5.71428*0.95, 1028.57143*16*4.523809*0.95, 200*32, 200*32*4.5016};
  for(int i = 0; i < 5; i++){
...
int main(int argc, char **argv)
{
  ...
  std::string gcode_out_2;
  node_handle.param("gcode_out_2", gcode_out_2, std::string("/gcode_out_2"));

  std::ofstream output_file(gcode_out);
  if(!output_file.is_open())ROS_ERROR_STREAM("Can't open " <<gcode_out);

  std::ofstream output_file_2(gcode_out_2);
  if(!output_file_2.is_open())ROS_ERROR_STREAM("Can't open " <<gcode_out);

  std::vector<KDL::Vector> find_end_effector_target_vol;
  find_end_effector_target_vol.reserve(501);

  std::vector<int> save_place;
  save_place.reserve(501);
  ...
  while(ros::ok()){
    //for(int x = -150;x <= 150;x++){
      ...
      for(int z = 301;z <= 450;z++){
        ...
        std::vector<KDL::JntArray> save_result(501);
        ...
        //}
        //use_onecore.clear();
        std::vector< geometry_msgs::Point > save_draw_point(501);
        save_draw_point.reserve(501);
        int times = 0;
        for(int l = 0;l < 501 ;l++){
          if(save_place[l] == 1){
        ...
        if(!save_draw_point.empty()){
          ...
          output_file << save_draw_point[times-1].x << "," << save_draw_point[times-1].y << "," << save_draw_point[times-1].z << std::endl;
          output_file_2 << save_draw_point[0].x << "," << save_draw_point[0].y << "," << save_draw_point[0].z << std::endl;
