...
void trac_ik_call(int &j,KDL::Frame &end_effector_pose,std::vector<KDL::JntArray> &save_result,int &second_execution,int &third_execution);
...
void trac_ik_call(int &j,KDL::Frame &end_effector_pose,std::vector<KDL::JntArray> &save_result,int &second_execution,int &third_execution){
  ...
  TRAC_IK::TRAC_IK tracik_solver3(chain_start, chain_end, urdf_param, 1, eps, TRAC_IK::Distance);
  KDL::Chain chain;
  KDL::JntArray ll, ul; //lower joint limits, upper joint limits
  if(!check_trac_ik_valid(tracik_solver, chain, ll, ul)) ROS_ERROR_STREAM("Error for limits 1");
  if(!check_trac_ik_valid(tracik_solver2, chain, ll, ul)) ROS_ERROR_STREAM("Error for limits 2");
  if(!check_trac_ik_valid(tracik_solver3, chain, ll, ul)) ROS_ERROR_STREAM("Error for limits 3");

  assert(chain.getNrOfJoints() == ll.data.size());
  assert(chain.getNrOfJoints() == ul.data.size());
  //ROS_INFO("Using %d joints", chain.getNrOfJoints());
  //ROS_INFO_STREAM("number:" << j);
  ...
  while(input_file){
    ...
    if(all_line % 1000 == 0 || !input_file){
      ...
      for(int j = 0;j < save_place.size();j++){
        ...
        trac_ik_call(j,end_effector_pose,save_result,second_execution,third_execution);
      }
      int pop_out = 0;
